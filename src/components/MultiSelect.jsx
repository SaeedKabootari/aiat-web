...
const MultiSelect = (props) => {
  ...
  useEffect(() => {
    const newChips = [];
    const coveredIds = new Set(); // IDs that should be covered by parent chips
    
    // First pass: identify all nodes that should be covered by parent chips
    selected.forEach(id => {
      const node = findNodeById(treeData, id);
      if (node && node.children && node.children.length > 0) {
        // This is a parent node - get all its descendants
        const descendants = getAllDescendantIds(node);
        descendants.forEach(descendantId => coveredIds.add(descendantId));
      }
    });

    // Second pass: create chips only for nodes not covered by parent selection
    selected.forEach((id) => {
      // Skip if this ID is covered by a parent selection
