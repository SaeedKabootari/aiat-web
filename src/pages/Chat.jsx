...
const ChatPage = () => {
  ...
  const handleSend = async (e) => {
    ...
    try {
      const res = await postActionSignalAx(`/api/chat`, { msg: inputValue }, { signal: abortController.signal });  // Corrected call
                ...
                <div className="prose prose-invert max-w-none">
                  <ReactMarkdown
                    ...
                    components={{
                      ...
                      blockquote: ({ node, ...props }) => (
                        <blockquote dir="rtl" className="border-r-4 border-gray-400 pr-4 my-4 text-gray-300" {...props} />
                      ),
                      hr: ({ node, ...props }) => <hr className="my-6 border-gray-600" {...props} />,
                      table: ({ node, ...props }) => (
                        <div className="overflow-x-auto">
                          <table dir="rtl" className="w-full my-4 border-collapse border border-gray-600" {...props} />
                        </div>
                      ),
                      th: ({ node, ...props }) => (
                        <th className="px-4 py-2 border border-gray-600 bg-gray-700 text-right" {...props} />
                      ),
                      td: ({ node, ...props }) => (
                        <td className="px-4 py-2 border border-gray-600 text-right" {...props} />
                      ),
      ...
      <footer className="bg-[#242752] p-4">
        <form onSubmit={handleSend} className="flex gap-2">
          ...
          <button
            ...
            className="bg-[#4f46e5] text-white rounded-xl w-12 h-12 flex items-center justify-center hover:bg-[#4338ca] transition-colors"
          >
            <svg xmlns="http://www.w3.org/2000/svg" className="h-6 w-6 rotate-270" fill="none" viewBox="0 0 24 24" stroke="currentColor">
